...
// -p -w 去掉只敲个空格的修改
git log -p -w --author=duststarr --pretty=tformat: --numstat -- . ":(exclude)miniprogram/colorui" ":(exclude)miniprogram/ec-canvas" ":(exclude)doc" | grep "\(.vue\|.css\|.wxss\|.js\|.wxml\|.wxs\|.md\)$" | awk '{ add += $1; subs += $2; loc += $1 - $2 } END { printf "added lines: %s, removed lines: %s, total lines: %s\n", add, subs, loc }'
